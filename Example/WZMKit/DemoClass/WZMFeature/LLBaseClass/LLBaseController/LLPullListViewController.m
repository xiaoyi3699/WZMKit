...
@interface LLPullListViewController ()

@end

@implementation LLPullListViewController{
    UIView *_badView;
}
...
- (void)didLoadDataWithResponseResult:(LLHttpResponseResult *)responseResult {
    ...
    if (responseResult.code == LLHttpResponseCodeSuccess) {
...
- (NSString *)badViewMessage {
    if (_superDataProvider.httpResponseResult.code == LLHttpResponseCodeFail) {
        return WZM_NO_NET;
    }
    return WZM_NO_NET;
}
