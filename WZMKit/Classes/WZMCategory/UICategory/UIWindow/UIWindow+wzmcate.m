...
@implementation UIWindow (wzmcate)
static NSString *_fpsKey = @"fps";
static NSString *_cpuKey = @"cpu";

- (UILabel *)fpsLabel
{
    return objc_getAssociatedObject(self, &_fpsKey);
}

- (void)setFpsLabel:(UILabel *)fpsLabel
{
    objc_setAssociatedObject(self, &_fpsKey, fpsLabel, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UILabel *)cpuLabel
{
    return objc_getAssociatedObject(self, &_cpuKey);
}
